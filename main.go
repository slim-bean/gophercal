...
func main() {
	fmt.Println("Starting gophercal")
	...
	case "run":
		...
		// We have a chicken and egg problem with the calendar and creating our oauth token, so we create it as
		// nil here first and will lazy load it in the handler when the token file exists.
		var cal *gcalendar.Calendar
		http.Handle("/dash.jpg", promhttp.InstrumentHandlerDuration(durationHistogram.MustCurryWith(prometheus.Labels{"handler": "dash.jpg"}), http.HandlerFunc(dashHandler(config, td, cal))))
...
func dashHandler(config *oauth2.Config, td todoist.Todoist, calendar *gcalendar.Calendar) func(w http.ResponseWriter, r *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {

		if calendar == nil {
			fmt.Println("making new calendar object")
			...
			calendar, err = gcalendar.NewCalendar(config, gopherCal.Run.GCalTokenFile, gopherCal.Run.GCalEmail)
			if err != nil {
				log.Println(err)
				http.Error(w, err.Error(), http.StatusInternalServerError)
				return
